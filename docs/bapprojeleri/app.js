...
function projCardHTML(proj, catColor, q) {
  ...
  return `
    <div class="proj-card" style="--cat-color:${catColor}" data-id="${esc(proj.id)}">
      <div class="proj-header" role="button" tabindex="0" aria-expanded="false" data-proj-toggle="${esc(proj.id)}">
...
async function init() {
  try {
    ...
    console.error("BAP projects load error:", err);
